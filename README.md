...
설치 프로그램은 설치 폴더를 고를 수 있으며 바탕 화면 바로가기는 첫 실행에서 선택합니다
...
첫 실행에서 **로컬 AI 설치**를 선택하거나 나중에 Electron 상단
**설정 → 로컬 AI 설치 및 모델 변경**을 누릅니다. 설치할 때만 모델 파일을 받기 위한
인터넷 연결이 필요하며, 설치가 끝난 뒤에는 로컬 번역과 로컬 음성 인식 자체에 인터넷이
필요하지 않습니다.

1. 앱이 시스템 RAM, GPU와 VRAM, 저장 공간을 확인해 기본 모델을 추천합니다.
2. 번역 모델과 음성 인식 모델을 각각 확인합니다.
3. Gemma 사용 조건을 열어 확인하고 동의합니다.
4. 모델을 영구 보관할 폴더와 임시 다운로드 폴더를 고릅니다.
5. 다운로드가 끝날 때까지 앱을 종료하지 않습니다. 중단되면 다음 실행에서 `.part`
   파일에 이어 받습니다.
6. SHA-256 검증과 압축 해제가 모두 끝나야 관리자 화면의 Local AI 선택지가 활성화됩니다.
...
번역은 `llama.cpp`의 Vulkan GPU 가속을 자동으로 사용하고 GPU를 쓸 수 없으면 CPU로
동작합니다. 음성 인식은 `whisper.cpp`와 기존 브라우저 VAD·PCM WebSocket 경로를 재사용해
...
> TranslateGemma는 지원 언어가 정해져 있으며 현재 싱할라어를 지원하지 않습니다.
> 싱할라어가 포함된 세션에서 Local AI 번역을 쓰려면 이를 지원하는 폴백 엔진을 명시적으로
> 고르세요. Local AI 번역에는 관리자 단어집이 적용되지 않습니다.
...
| Local AI (TranslateGemma) | 인터넷·API 키 없이 컴퓨터 안에서 번역 | 설치형 전용, 모델 메모리 필요, 싱할라어·단어집 미지원 |
...
**설정 → 로컬 AI 설치 및 모델 변경**에서 모델 설치를 완료하세요. Local AI에는 API 키를
등록하지 않습니다.
