...
[v0.9.13 릴리스 페이지](https://github.com/Levi02114/LiveConfTranslation/releases/tag/v0.9.13)에서 다음 중
하나를 내려받습니다.
...
고릅니다. 통합 입력 주소는 설정 저장 뒤 세션의 페이지 URL 목록에 나타납니다.

- 타자로 보낸 문장은 OpenAI 언어모델이 활성 입력 언어 중 하나로 판정한 뒤 저장합니다.
...
통합 입력의 타자 언어 감지와 음성 전사는 모두 OpenAI API 키가 필요합니다. 키가 없거나
감지 호출이 실패하면 타자는 기본 언어로 계속 입력할 수 있지만, 마이크는 시작할 수 없습니다.
...
마이크 인식 정확도를 높이려면 운영 설정의 **전사 컨텍스트**에 안건, 분야, 화자 이름,
자주 쓰는 용어를 적어 두세요. 입력·캡처·통합 입력 모든 마이크 세션의 전사 프롬프트에
반영됩니다. 페이지 구조가 아니라 음성 인식에만 영향을 주므로 운영 설정이 잠긴 뒤에도
바꿀 수 있으며, 다음 마이크 시작부터 적용됩니다.
