...
현재 배포 버전: [v0.9.24](https://github.com/Levi02114/LiveConfTranslation/releases/tag/v0.9.24)
...
2. 번역 키는 세션 생성 영역의 **API 키 등록**을 누릅니다.
...
Google 음성 인식은 API 키 등록 창이 아니라 **음성 인식 엔진 → Google Cloud
Speech-to-Text → 서비스 계정 등록**에서 JSON 파일을 등록합니다. 서비스 계정의 평문
...
Windows Electron 앱 상단의 **설정 → 앱 설명 실행**을 누르면 현재 관리자 화면 위에서
실제 버튼과 설정 영역을 차례로 강조하는 안내가 시작됩니다.

Electron 메뉴도 관리자 화면에서 선택한 표시 언어를 따릅니다. Windows 설치형은 설치
...
1. **개인 채팅 연결**을 누르면 등록한 봇의 Telegram 대화가 열립니다.
2. Telegram에서 **Start/시작**을 누릅니다. 앱은 일회용 연결 코드가 포함된 명령만 받습니다.
...
연동을 마친 뒤에는 Electron의 **설정 → Cloudflare 터널 자동 시작·복구** 체크 항목에서
...
- Electron의 **설정 → Telegram URL 알림…**에서 봇 확인 상태와 수신 대상 확인
