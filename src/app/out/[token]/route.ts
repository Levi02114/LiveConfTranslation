...
scroll.addEventListener('scroll',()=>{stick=scroll.scrollHeight-scroll.scrollTop-scroll.clientHeight<=80;if(stick){pending=0;latest.hidden=true}},{passive:true});latest.onclick=()=>{stick=true;pending=0;latest.hidden=true;scroll.scrollTop=scroll.scrollHeight};
...
applyTheme();size.textContent=labels[sizes.indexOf(root.dataset.size||'md')];scroll.scrollTop=scroll.scrollHeight;if(!DATA.closed)connect();else connection.textContent=DATA.strings.disconnected;
`;
