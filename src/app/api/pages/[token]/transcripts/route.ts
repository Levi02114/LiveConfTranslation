...
import { transcriptionSignal } from "@/lib/realtime/transcription-registry";
import {
  getMeeting,
...
export async function POST(request: Request, { params }: Params) {
  ...
  const fingerprint = inputFingerprint(parsed.data.lang ?? page.lang, parsed.data.body, meeting.speakerLabels ? parsed.data.speakerName : null);
  const existing = findMessageRetry(meeting.id, page.id, parsed.data.ingestKey, fingerprint);
  if (existing) return Response.json({ message: existing, inserted: false });
  ...
  const rewritten = parsed.data.rewrite
    ? await rewriteTranscript({
        ...
        signal: transcriptionSignal(parsed.data.leaseId),
      })
    : cleaned;
  const body = cleanTranscript(rewritten) ?? cleaned;
  if (!ownsCapture(page.id, parsed.data.leaseId)) {
    return Response.json({ error: "capture-expired" }, { status: 409 });
  }
