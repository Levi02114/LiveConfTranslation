...
export async function GET(request: Request, { params }: Params) {
  ...
  if (afterParam !== null && Number.isSafeInteger(after) && after >= 0) {
    return Response.json(
      {
        entries: history.filter((entry) => entry.messageId >= after).map(compact),
  ...
  const data = json({
    token,
    ids: history.map((entry) => entry.messageId),
    closed: meeting.status === "closed",
...
function renderEntry(entry: CombinedEntry, languages: Language[], strings: UiStrings): string {
  ...
  const targets = languages
    ...
    .map((language) => {
      ...
      return `<div class="translation" data-lang="${html(language.code)}" lang="${html(language.code)}" dir="${textDirection(language.code)}"><div class="label">${html(language.nativeName)}</div><p class="${status}">${html(body)}</p></div>`;
    })
    .join("");
...
main{height:100%;overflow-y:auto;padding:0 32px}.wrap{max-width:1400px;margin:auto;padding-top:3.375rem}header{border-bottom:1px solid var(--line);padding-bottom:20px}h1{margin:0 0 6px;font-size:1.6875rem;font-weight:500}header div,#empty,.label{color:var(--muted);font:.75rem ui-monospace,monospace}
...
scroll.addEventListener('scroll',()=>{stick=scroll.scrollHeight-scroll.scrollTop-scroll.clientHeight<=80;if(stick){pending=0;latest.hidden=true}},{passive:true});latest.onclick=()=>{stick=true;pending=0;latest.hidden=true;follow()};
...
applyTheme();size.textContent=labels[sizes.indexOf(root.dataset.size||'md')];follow();if(!DATA.closed)connect();else connection.textContent=DATA.strings.disconnected;
`;
