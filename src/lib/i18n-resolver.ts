// Also used by Electron's embedded server; repo.ts enforces the Node boundary.

import {
  applyStrings,
  type AdminStrings,
  BUILTIN_ADMIN,
  BUILTIN_ADMIN_ADDITIONS,
  BUILTIN_UI,
  FALLBACK_ADMIN,
  FALLBACK_UI,
  flattenStrings,
...
export function getAdminStrings(lang: LanguageCode): AdminStrings {
  ...
  const additions = BUILTIN_ADMIN_ADDITIONS.get(lang);
  return applyStrings(additions ? applyStrings(base, flattenStrings(additions)) : base, loadOverlay(lang, ADMIN_PREFIX));
}
...
export function resolveEntries(lang: LanguageCode): ResolvedEntry[] {
  ...
  const builtinAdmin = admin ? flattenStrings(admin) : flattenStrings(BUILTIN_ADMIN_ADDITIONS.get(lang) ?? {});
