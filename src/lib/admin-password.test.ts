...
test("변경한 관리자 비밀번호는 저장되고 이전 세션은 무효화된다", async () => {
  ...
  try {
    ...
    assert.equal(await verifyAdminPassword("initial-password"), true);
    assert.equal(await changeAdminPassword("wrong-password", "new-password-123"), "invalid-current");
    assert.equal(await changeAdminPassword("initial-password", "short"), "too-short");
    assert.equal(await changeAdminPassword("initial-password", "initial-password"), "same-password");
    assert.equal(await changeAdminPassword("initial-password", "new-password-123"), "ok");
    assert.equal(await verifyAdminPassword("initial-password"), false);
    assert.equal(await verifyAdminPassword("new-password-123"), true);
...
test("손상된 비밀번호 파일은 초기 비밀번호로 폴백하지 않는다", async () => {
  ...
  try {
    ...
    await assert.rejects(() => verifyAdminPassword("initial-password"));
  } finally {
